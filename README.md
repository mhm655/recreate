...
- **"Same output" is subtle.** Plain JSON can't tell a wrong answer from a right one when they differ only in `NaN`, `-0` or `undefined`, and it breaks on `Map`, `Date` and cycles. Results travel in a [lossless tagged encoding](#tagged-encoding), so return values and thrown errors are compared exactly, and so is what the function did to its own arguments.
...
- **Values are edge-first.** `valuesFor` (`src/generator/values.ts`) returns things like `''`, long strings, `0`, negative numbers, `NaN`/`Infinity`/`-Infinity`, empty/singleton containers, and an invalid `Date`, not just one "normal" example -- the point of a fixed suite is to catch a rewrite that only handles the easy inputs. Arrays also come in both orders: the longest generated array uses distinct elements and is repeated reversed, so sorting, reversing or dedupe-keeping-first behaviour always sees an out-of-order input. (Random picks alone once produced an already-sorted array, and an in-place sort then couldn't be told apart from a rewrite that returned a sorted copy.)
...
- **Arguments after the call count too.** What a function does to the objects it was handed is observable behaviour. A rewrite of an in-place `sortScores(xs)` that returns a sorted copy has the right return value and leaves the caller's array untouched, so it fails with *"same result, but left its arguments in a different state"*, and the report shows both argument states. The reverse also fails: mutating inputs the original left alone. Challenges store each test's post-call arguments (`expectedArgsAfter`). Challenges captured before that field existed are graded on outcomes alone, as they were when captured.
