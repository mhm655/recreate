...
- **A handful of reserved marker keys (`__truncatedLength`, `__truncatedKeys`, `__droppedSymbolKeys`) are indistinguishable in the wire format from a real own property of the same name.** `encoding.ts` hangs these off an array/object/error node to record that a budget cap dropped something; `decode()` unconditionally skips any entry with one of these exact keys. A submission's real object that happens to own a property with one of these names loses it on decode. Narrow (nobody names a property `__truncatedKeys` by accident) but real; the actual fix is moving these markers to dedicated sibling fields on the node instead of commingling them with real entries, which is a wire-format change with its own migration story for already-captured Challenge JSON, so it's left as a known gap rather than done under this pass.
- **`recordCalls`'s own encoding work isn't bounded by the ambient encode budget.** Each recorded entry's `args` gets its own fresh `Encoder`/node-count (see the code comment on why it must be fresh), so a large `recordCalls` table can do up to `maxCollectionEntries` times as much encoding work as the budget's `maxNodes` is meant to cap for one call. Not attacker-reachable -- `recordCalls` is a trusted test-authoring API, called by whoever writes the test, not by submitted code -- but worth knowing if a captured suite's `recordCalls` inputs are unexpectedly large.
